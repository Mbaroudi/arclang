...
function validateSystemBoundary(
  ...
  // Check for system boundary - can be a node OR indicated in metadata
  const systemNode = nodes.find(n => 
    n.metadata?.is_system === true ||
    n.metadata?.type === 'system'
  );
  
  // Also check if boundary is rendered externally (Phase 1 implementation)
  const hasBoundaryRendered = nodes.some(n => n.metadata?.system_boundary_rendered === true);

  if (!systemNode && !hasBoundaryRendered) {
  ...
  // If boundary is rendered externally (Phase 1), give full score
  if (hasBoundaryRendered) {
    return {
      score: 100,
      weight: METRIC_WEIGHTS.CRITICAL,
      pass: true,
      ...
      threshold: 1,
      violations: [],
      description: 'System boundary rendered',
    };
  }

  // Validate system node if present
  const bounds = calculateBounds(nodes);
...
function validateColorCompliance(
  ...
  // Updated to Capella-compliant colors (Phase 1 - Table 6)
  const colorRules: Record<string, string> = {
    activity: '#FFB266',      // Operational Activity (CORRECTED)
    actor: '#FFFF99',          // Operational Actor/Entity (CORRECTED)
    function: '#ADD8E6',       // System Function (CORRECTED)
    component: '#6495ED',      // Logical Component (CORRECTED)
    behavior: '#4169E1',       // Physical Behavioral Component (CORRECTED)
    'physical-node': '#FFD700',// Physical Node/Hardware (CORRECTED)
    hardware: '#FFD700',       // Physical Hardware (alias)
  };
...
function validateComponentNesting(
  ...
  const physicalNodes = nodes.filter(n => n.type === 'physical-node' || n.metadata?.nodeType === 'physical');
  const behavioralComponents = nodes.filter(n => n.metadata?.componentType === 'Behavior' || n.metadata?.type === 'behavioral');
